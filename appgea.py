...
uploaded_file = st.file_uploader("Faça upload de um arquivo CSV (.csv)", type=["csv"])
...
if uploaded_file is not None:
    try:
        ...
        if not expected_columns.issubset(df_estoque.columns):
            st.error("O arquivo CSV não contém as colunas esperadas: 'ingrediente', 'quantidade_atual', 'consumo_medio_diario', 'limite_minimo'.")
            st.stop()
